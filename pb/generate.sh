...
PROTO=${1:-"$ROOT/../proto"}
PROTO_ETHEREUM=${2:-"$ROOT/../firehose-ethereum/proto"}
PROTO_EVM_EXECUTOR=${2:-"$ROOT/proto"}
...
function main() {
  ...
  generate "sf/ethereum/statedb/v1/" "statedb.proto" "singlet.proto"
  ...
  echo "streamingfast/firehose-ethereum/proto revision: `GIT_DIR=$PROTO_ETHEREUM/../.git git log -n 1 --pretty=format:%h -- proto`" >> $ROOT/pb/last_generate.txt
  echo "streamingfast/evm-executor/proto revision: `GIT_DIR=$ROOT/.git git log -n 1 --pretty=format:%h -- proto`" >> $ROOT/pb/last_generate.txt
}
...
function generate() {
    ...
    for file in "$@"; do
      protoc -I$PROTO -I$PROTO_ETHEREUM -I$PROTO_EVM_EXECUTOR \
